...
class NSGA_II_1v():
    def __init__(self, dataset, name, time, seed, split, experiment,lb_f, ub_f, lb_w, ub_w, lb_b, ub_b, pm_1, pm_2):
        self.lb_f = lb_f
        self.ub_f = ub_f
        self.lb_w = lb_w
        self.ub_w = ub_w
    ...
    def loss(self):
        ...
        for i in self.population:
            if(self.pareto_f[i]==1):
                ...
                ind_loss=log_loss(y,probabilidades)
                loss_mean+=ind_loss
                if(ind_loss<mini):
                    mini=ind_loss
    ...
    def plot_loss(self):
        ...
        for i in self.times:
            text+=str(i)+' '
        with open(f'Genetico/Outputs/{self.experiment_name}/times_{self.name}_{str(self.time)}_{self.seed}.txt', 'w') as file:
            file.write(text)
    ...
    def individual(self):
        f=np.random.randint(max(4,self.lb_f*self.n_features), max(6,self.ub_f*self.n_features))

        sv=[np.random.uniform(self.lb_w,self.ub_w) for i in range(f)]
        b=np.random.uniform(self.lb_b,self.ub_b)
    ...
    def mutation(self,ind):
        if(len(ind[0])==0):
            pass
            #print('perdido')
        if(np.random.random()<self.pm_1*0.999**self.iter):
    ...
    def mutation_nf(self, ind):
        if(np.random.random()<self.pm_2*0.999**self.iter):
            f=len(ind[2])
    ...
    def main(self):
        ...
        while(True):
            ...
            self.loss()
            self.times.append(time.time()-past)
